...
{
  if(USART_GetFlagStatus(USART2,USART_FLAG_TC)!=RESET);//如果是发送完成中断就执行以下语句
   {
      if(USART_GetITStatus(USART2,USART_FLAG_TC)!=RESET)//如果发送中断挂起中断置位
	    {		   
		   USART_ClearITPendingBit(USART2,USART_FLAG_TC);//清除挂起的中断标志
		}

   }
  if(USART_GetFlagStatus(USART2,USART_IT_RXNE)!=RESET);//如果是接收完成中断就执行以下语句
   {
      if(USART_GetITStatus(USART2,USART_IT_RXNE)!=RESET)//如果接收中断挂起中断置位
	    {		   
		   USART_ClearITPendingBit(USART2,USART_IT_RXNE);//清除挂起的中断标志
		}
	  test_value=(unsigned char)USART_ReceiveData(USART2);//将接收到得数据赋值给test_value
	  flag=1;//将标志置位
