/**************************************************************************************
函数功能：USART2函数   发送  接收中断实现
          注意串口2  挂在低速外设总线上  同时PA2  TX    PA3  RX   
**************************************************************************************/
#include<stm32f10x.h>
#include<misc.h>
...
void GPIO_Configration()
{
   ...
   GPIO_InitStruct.GPIO_Pin  = GPIO_Pin_2;//将USART1的TX设置为
   ...
   GPIO_InitStruct.GPIO_Pin  = GPIO_Pin_3;//将USART1的TX设置为
...
void NVIC_Configration()
{
  ...
  NVIC_InitStruct.NVIC_IRQChannel=USART2_IRQChannel;//设置中断的通道为外部中断3
...
void  USART_Configration()
{
   ...
   USART_DeInit(USART2);//UART1的寄存器初始化
   ...
   USART_ITConfig(USART2,USART_IT_RXNE,ENABLE);//允许相应的串口1的接收中断  
   USART_Cmd(USART2,ENABLE);//允许USART1开始工作	
			     ...
			     case(1):
						 {
							  ...
							  {
							    USART_SendData(USART2,u1test[i]);
								while(USART_GetFlagStatus(USART2,USART_FLAG_TC)==RESET);//等待发送完成
								USART_ClearFlag(USART2,USART_FLAG_TC);//清除发送完中断标志
							  }
					     }break;
				 case(0):
						 {
							  ...
							  {
							    USART_SendData(USART2,u2test[i]);
								while(USART_GetFlagStatus(USART2,USART_FLAG_TC)==RESET);//等待发送完成
								USART_ClearFlag(USART2,USART_FLAG_TC);//清除发送完中断标志
							  }
					     }break;
